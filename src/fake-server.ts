...
export async function fakeServer(host: string, callback: (address: any) => void) {
  ...
  fake.on('listening', () => {
    logger.verbose('fake listen success: %o', address)
    callback(address)
  })
  ...
  fake.on('tlsClientError', (err: Error, socket: TLSSocket) => logger.error('tls client error: %o', err))

  fake.on('error', err => logger.error('fake server error: %o', err))
